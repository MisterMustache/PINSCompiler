...
public class TypeChecker implements Visitor {
    ...
    private final HashSet<TypeName> typeNames;
    ...
    public void visit(Call call) {
        if (this.types.valueFor(call).isEmpty()) call.accept(this);

        if (this.types.valueFor(call).isEmpty())
            Report.error(call.position, "SEM: Type of 'fun' is not present.");

        if (this.types.valueFor(call).get().asFunction().isEmpty())
            { Report.error("SEM: Unexpected error. 'fun' is not present."); return; }
        var function = this.types.valueFor(call).get().asFunction().get();

        if (function.parameters.size() != call.arguments.size()) Report.error(call.position, "SEM: The number" +
                " of passed arguments does not match the number of required parameters of function '" + call.name + "'.");

        var parameters = function.parameters;
        var arguments = call.arguments;
        for (int i = 0; i < parameters.size(); i++) {
            if (this.types.valueFor(arguments.get(i)).isEmpty()) arguments.get(i).accept(this);
            if (this.types.valueFor(arguments.get(i)).isEmpty())
            ...
            if (!this.types.valueFor(arguments.get(i)).get().equals(parameters.get(i)))
                Report.error(arguments.get(i).position, "SEM: Type mismatch. Expected '" + parameters.get(i) +
                        "', got '" + this.types.valueFor(arguments.get(i)).get() + "'.");
        }
    ...
    public void visit(Binary binary) {
        ...
        if (left.isLog() && right.isLog()) {
            switch (binary.operator) {
                case AND, OR, EQ, NEQ, LEQ, GEQ, LT, GT -> this.types.store(left, binary);
            ...
            switch (binary.operator) {
                case ADD, SUB, MUL, DIV, MOD -> this.types.store(left, binary);
            ...
            Report.error(binary.position, "SEM: Incompatible types '" + left + "' and '" + right + "'.");
        }
    ...
    public void visit(For forLoop) {
        ...
        if (this.types.valueFor(forLoop.low).get().isInt() ||
            ...
            Report.error(forLoop.position, "SEM: Type mismatch. All 'for' components must be of type '" +
                    new Type.Atom(Type.Atom.Kind.INT) + "'.");
    }
    ...
    public void visit(Array array) {
        // Array potrebuje tip
        // Če še ni izračunan ga izračuna
        if (this.types.valueFor(array).isEmpty()) array.type.accept(this);

        // Če tip Array-a še zmeraj ne obstaja potem je to napaka
        if (this.types.valueFor(array).isEmpty())
            Report.error(array.type.position, "SEM: Type of 'ARR' not present.");

        // Shrani tip Array-a
        this.types.store(new Type.Array(array.size, this.types.valueFor(array).get()), array);
    }
    ...
    public void visit(TypeName name) {
        ...
        if (this.typeNames.contains(name))
            Report.error(name.position, "SEM: Type of 'typ' must not reference back to self.");
        ...
        if (type.isEmpty()) name.accept(this);
